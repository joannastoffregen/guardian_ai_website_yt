...
### Subheadline  
Continuous protection with anomaly detection, suspicious login alerts, and automated security rules customized for your infrastructure.
...
### Hero Visual  
- 3D guardian angel statue with dramatic lighting  
- Use this exact code from three.js  
- Dark swirling background texture  

## Components and Design Instructions

Text Design
- The text for all sections include hero should be like this: 
- Tag on top: (this is not applicable to hero section)
- Headline; use white Shatoshi fonts, and Instrument serif Regular Italic for 1-2 accent works in color #01FFFF 
- Subheadline: Satoshi fonts with off white / slightly greyish 
...
Hero Visual 
- I want the 3D guardian angel statue with dramatic lighting behind the text and in the middle. 
- The 3d animation should take the whole width of the hero section.
- Dark swirling background texture 

- ### 3D Interactivity
- As a user I should be able to click and drag the 3D model to rotate it 360 degrees
- Enable smooth damping (enableDamping) for natural, weighted movement
- Enable zoom (scroll wheel) and pan (right-click drag)

 **Critical implementation note:** 
When layering text/UI over the 3D canvas, use `pointer-events-none` on the overlay container and `pointer-events-auto` on interactive elements (buttons, links) to allow mouse events to pass through to the 3D scene
- Use this exact code from three.js for the 3d animation https://github.com/mrdoob/three.js/blob/master/examples/webgl_lights_spotlight.html

Code for the 3d animation: 
<!DOCTYPE html>
<html lang="en">
	<head>
...
Button component 
- Use this component for the Get started button: 
- Should be pill shaped and dark background with glow outline. 
- Use this component for the signup button, but the glowing outline to be in our main teal/cyan color. 
- Url: https://21st.dev/community/components/erikx/star-button/default 
- Code: 
"use client";
...
- Use a frosted blass button for View Docs 
- Add on hover effect and relevant icon next to the text 
